...
unknown tool name never executes. Today the default registry holds the read-only
app tools; mutating tools arrive in a later phase and must declare a non-``read``
risk level plus approval handling.
"""
...
def get_tool_registry(settings: Settings | None = None) -> ToolRegistry:
    """Return the cached default registry of read-only app tools.
